...
def print_free_memory(device): 
    ...
    print(f"Total memory on device {device}: {total_memory/1024**2}")
    print(f"Free memory on device {device}: {free_memory/1024**2}")
...
def set_save_path(save_path, remove=True):
    ...
    ensure_path(os.path.join(save_path, 'tensorboard'), remove=remove)
...
def calc_iou(pred, target):
    ...
    # torch.Size([1, 100, 1024, 1024]) torch.Size([1, 100, 1024, 1024])
    ...
    target = (target >= 0.5).to(torch.int)
    # print(pred.max(), pred.min())
    inter = (pred * target).sum(dim=(2, 3))
    union = (pred + target).sum(dim=(2, 3)) - inter
    iou_thresholded = (inter / union)
    iou_score_thresholded = iou_thresholded.mean().item()

    # Initialize metrics
    # pred=pred.to("cpu")
    # target=target.to("cpu")
    # iou_metric = IoU(num_classes=2, threshold=0.5, ignore_index=0)
    # with torch.no_grad():
    #     iou = iou_metric(pred, target)
    # iou_score_IoUfunc = iou.mean().item()

    return iou_score, iou_score_thresholded,  np.array(0), np.array(0)
