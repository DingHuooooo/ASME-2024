...
class SAM_CNN(nn.Module):
    ...
    def forward(self):
        ...
        self.mask_generator_out = self.mask_generator(self.input)
    ...
    def infer(self, input):
        ...
        # Embed prompts (with mask)
        self.mask_generator_out = self.mask_generator(input)
        self.mask_generator_out = self.mask_generator_out.mean(dim=1, keepdim=True)
        self.sparse_embeddings, self.dense_embeddings = self.prompt_encoder(
                points=None,
                boxes=None,
    ...
    def optimize_parameters(self):
        ...
        self.backward()  # calculate graidents for Gself.optimizer.step()  # update G's weights
        del self.features, self.input, self.mask_generator_out, self.low_res_masks, self.iou_predictions, self.sparse_embeddings, self.dense_embeddings
        torch.cuda.empty_cache()  # clear CUDA cache
        self.optimizer.zero_grad()  # set G's gradients to zero again
...
class SAM_ADAPTER(nn.Module):
    ...
    def forward(self):
        ...
        self.sparse_embeddings = torch.empty((bs, 0, self.prompt_embed_dim), device=self.input.device)
        self.dense_embeddings = self.no_mask_embed.weight.reshape(1, -1, 1, 1).expand(
    ...
    def infer(self, input):
        ...
        # Embed prompts
        self.sparse_embeddings = torch.empty((bs, 0, self.prompt_embed_dim), device=input.device)
        self.dense_embeddings = self.no_mask_embed.weight.reshape(1, -1, 1, 1).expand(
            bs, -1, self.image_embedding_size, self.image_embedding_size
        )
    ...
    def optimize_parameters(self):
        ...
        del self.features, self.input, self.low_res_masks, self.iou_predictions, self.sparse_embeddings, self.dense_embeddings
        torch.cuda.empty_cache()  # clear CUDA cache
        self.optimizer.zero_grad()  # set G's gradients to zero again
