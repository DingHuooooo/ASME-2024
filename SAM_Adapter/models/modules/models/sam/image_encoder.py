...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        self.adaptor = Adaptor(self.scale_factor, self.prompt_type, self.embed_dim,
                                                self.tuning_stage, self.depth,
                                                self.input_type, self.freq_nums,
                                                self.handcrafted_tune, self.embedding_tune,
                                                img_size, patch_size)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Here adds the Adaptor to x
        embedding_feature = self.adaptor.init_embeddings(x)
        handcrafted_feature = self.adaptor.init_handcrafted(inp)
        prompt = self.adaptor.get_prompt(handcrafted_feature, embedding_feature)
...
class Adaptor(nn.Module):
    def __init__(self, scale_factor, prompt_type, embed_dim, tuning_stage, depth, input_type,
                 freq_nums, handcrafted_tune, embedding_tune, img_size, patch_size):
        """
        Args:
        """
        ...
        self.feature_generator = PatchEmbed2(img_size=img_size,
                                                   patch_size=patch_size, in_chans=3,
                                                   embed_dim=self.embed_dim//self.scale_factor)

    # only the under 3 functions are used
